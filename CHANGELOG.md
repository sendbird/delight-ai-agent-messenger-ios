...
- Fixed the message input staying disabled longer than intended after sending a message (#615)
- Improved AI Agent stats stability by bounding deduplication key retention (#618)
...
- **Added `SBATheme` public component interfaces** — All theme sub-classes and their
  color/font properties are now publicly accessible for customization:
  - `SBATheme.Conversation` — `header`, `list`, `input` and all nested properties
  - `SBATheme.ConversationList` — `header`, `list` and all nested properties
  - `SBATheme.Common` — `actionSheet`, `toast`, `alert`, `loading`, `emptyView`, `markdown`, etc.
  - `SBATheme.Launcher` — icon tint, background colors
  - `SBATheme.User` — nickname, userId, profile colors/fonts

- **Added `SBAThemeColor` public interface** — Color property wrapper now provides
  `update(with:type:)`, `update(light:dark:)`, `update(all:)`, `set(colorScheme:)`
  for per-property color overrides

- **Added `SBAThemeFont` public interface** — Font property wrapper now provides
  `resetToDefault()` and `isCustomized` for managing font customization state

- **Added `SBAIconSet` public icon properties** — All UI icons are now accessible and
  replaceable via public static properties (e.g. `iconSend`, `iconClose`, `iconCamera`, ...)
